...
ifeq ($(PLATFORM),linux)
	...
	CFLAGS=`sdl-config --cflags` `pkg-config zziplib libconfuse gl glu lua5.1 --cflags` -Werror -Wall -ggdb
	LIBS=`sdl-config --libs` `pkg-config zziplib libconfuse lua5.1 --libs` -lGL -lGLU -lGLEW -lSDL_mixer -lSDL_image
endif
