...
bool Mod::load(UIUpdate* ui)
{
	...
	// Auto create pickups for the weapons, using a autogenerated model filename
	// and falling back to to a generic filename if no file is found
	...
	for (int i = weapontypes->size() - 1; i >= 0; --i) {
		...
		AssimpModel *model = this->getAssimpModel("pickup_weapon_" + tmp + ".dae");
	...
	// Auto create pickups for the weapon ammo, using a autogenerated model filename
	// and falling back to to a generic filename if no file is found
	AssimpModel *generic_ammo = this->getAssimpModel("pickup_ammo_generic.dae");
	for (int i = weapontypes->size() - 1; i >= 0; --i) {
		string tmp = "ammo_" + weapontypes->at(i)->name;

		AssimpModel *model = this->getAssimpModel("pickup_ammo_" + tmp + ".dae");
