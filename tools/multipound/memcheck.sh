...
while [ 1 ]; do
	...
	# Run the test.
	valgrind --leak-check=yes ./chaoticrage --no-mouse-grab --campaign=pound --mod=test $@ 2>&1 | tee $LOGFILE
	...
	# If it coredumped, save the file somewhere useful
	if [ -f $LOGFILE ]; then
		mkdir -p tools/multipound/memcheck
		mv $LOGFILE tools/multipound/memcheck/log_`date +"%Y-%m-%d_%H-%M-%s"`
	fi
